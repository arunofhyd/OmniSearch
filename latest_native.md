...
on run {input, parameters}
	with timeout of 30 seconds
		...
		if not isResetCommand then
			...
			set isGL to false
			try
				set isGL to (do shell script "python3 -c \"import sys, re; t=sys.argv[1].lower().strip(); print('true' if re.search(r'\\b(guidelines?|gls?)\\b|📋|9\\.\\s*guidelines?', t) else 'false')\" " & quoted form of searchTerm) is "true"
			end try
			
			if isGL or lowerTerm contains "guideline" or lowerTerm contains "gl" or lowerTerm contains "📋" then
