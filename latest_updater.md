...
on run {input, parameters}
	with timeout of 300 seconds
		...
		set localeData to {¬
			...
			"en_ZA|🇿🇦", "en_SG|🇸🇬", "ar_SA|🇸🇦", "hi_IN|🇮🇳", "zh_CN|🇨🇳", "zh_HK|🇭🇰", "ja_JP|🇯🇵"}
		...
		if isFirstRun then
			tell application (path to frontmost application as text)
				...
				if button returned of welcomeResponse is "Let's Go!" then
					...
					set chosenEnginesList to choose from list engineOptions with prompt ("Select the search engines you want to enable:" & return) default items {item 1 of engineOptions} with title "OmniSearch Setup (5/6)" with multiple selections allowed
					...
					if chosenEnginesList is not false then
						repeat with chosenEng in chosenEnginesList
							...
							set chosenLocales to choose from list locOptions with prompt ("Select regions for " & engName & ":" & return) default items {item 2 of locOptions} with title "OmniSearch Setup (6/6)" with multiple selections allowed
